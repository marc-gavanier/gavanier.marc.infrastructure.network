resource "aws_route53_zone" "domainNames" {
...
resource "aws_route53_record" "main_name_servers_record" {
  for_each        = toset(local.domainNames)
  name            = aws_route53_zone.domainNames[each.key].name
  allow_overwrite = true
  ttl             = 30
  type            = "NS"
  zone_id         = aws_route53_zone.domainNames[each.key].zone_id
  records         = aws_route53_zone.domainNames[each.key].name_servers
}
...
resource "aws_route53_record" "certificate_validation_main" {
  ...
  zone_id         = aws_route53_zone.domainNames[each.key].id
  type            = "CNAME"
  ttl             = "300"
...
resource "aws_route53_record" "ipv4_record" {
  ...
  alias {
    name                   = aws_cloudfront_distribution.web_front_hosting.domain_name
    zone_id                = aws_cloudfront_distribution.web_front_hosting.hosted_zone_id
